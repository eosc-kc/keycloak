...
import org.keycloak.representations.openid_federation.PolicyList;
...
//        //add also the leaf self-signed node -No needed, leaf is the first authority hint
//        trustChainForExplicits.forEach(trustChainForExplicit -> trustChainForExplicit.getParsedChain().add(0, leafEs));
...
//                    }
    ...
    private List<TrustChainForExplicit> subTrustChains(EntityStatement leafEs, List<String> trustAnchorIds, Set<String> visitedNodes) {
        ...
        if (leafEs.getAuthorityHints() != null && !leafEs.getAuthorityHints().isEmpty()) {
            ...
            chainsList.add(new TrustChainForExplicit());
        }
...
//
    ...
    public EntityStatement parseAndValidateSelfSigned(String token) throws InvalidTrustChainException {
        ...
        try{
            ...
            ConfigurableJWTProcessor<SecurityContext> jwtProcessor = new DefaultJWTProcessor<>();
            //jwtProcessor.setJWSTypeVerifier(new DefaultJOSEObjectTypeVerifier<>(new JOSEObjectType("JWT")));
            JWSKeySelector<SecurityContext> keySelector = new JWSVerificationKeySelector<>(Stream.of(JWSAlgorithm.RS256).collect(Collectors.toSet()), keySource);
            jwtProcessor.setJWSKeySelector(keySelector);
