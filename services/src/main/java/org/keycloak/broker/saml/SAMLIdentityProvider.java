...
//            }
            ...
            if (getConfig().isPassSetMfa() && requestedLoaNumber > (previouslyAuthenticatedNote == null ? NO_LOA : Integer.parseInt(previouslyAuthenticatedNote))) {
                Entry<String, Integer> loaEntry = AcrUtils.getAcrLoaMap(request.getAuthenticationSession().getClient()).entrySet().stream().filter(entry -> entry.getValue().equals(requestedLoaNumber)).findFirst().get();
                if (loaEntry != null) {
                    authnContextClassRefs.stream().forEach(x -> requestedAuthnContext.addAuthnContextClassRef(x));
